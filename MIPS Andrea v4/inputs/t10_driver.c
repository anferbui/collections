...
{	
	...
	Driver_LoadInstructions(argc>1 ? argv[0] : "t10_input-mips.bin", (uint32_t*)pMem);
	...
	for(i=0;i<100;i++){
		uint32_t a=rand(), b=rand(), c=rand(), d=rand(), res_correct, res_sim;
		...
		mips_set_register(state, R_a1, d);
		...
		// Get the true result
		res_correct=t10_F(a,b,c,d);
	
		if(res_sim!=res_correct){
			fprintf(stdout, "F(%u,%u,%u,%u), sim=%u, correct=%u\n", a, b, c, d, res_sim, res_correct);
			fail=1;
		}
	}
